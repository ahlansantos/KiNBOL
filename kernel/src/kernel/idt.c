 ...
 * exception stubs that either push a dummy error code or leave the CPU's
 * real one alone), and the dispatcher that decides whether something is
 * a fatal exception (vector < 32) or a real IRQ with a registered
 * handler.
 *
 * This is also where the #GP bug used to live: the stubs for
 * error-code-pushing exceptions (8, 13, 14) were pushing an extra dummy
